...
const RecentMessage = () => {
  ...
  return (
    <div className='bg-white max-w-xs mt-4 p-4 min-h-40 rounded-md shadow text-xs text-slate-800' >
        <h3 className='font-semibold text-slate-800 mb-4'>Recent Message</h3>
        <div className='flex flex-col max-h-56 overflow-y-scroll no-scrollbar'>
            {
                message.map((message, index) =>(
                    <Link to={`/messages/${"message.from _user_id._id"}`} key={index} className='flex items-start gap-2 py-2 hover:bg-slate-700'>
