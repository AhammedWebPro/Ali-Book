...
const StViewer = ({ viewStory, setViewStory }) => {
    ...
    const renderContent = () =>{
        switch (viewStory.media_type) {
            ...
            case "text":
                return (
                    <div className='w-full h-full flex items-center p-8 text-white text-2xl text-center'>
                        {viewStory.content}
    ...
    return (
        <div className='fixed inset-0 h-screen bg-black bg opacity-90 z-110 flex items-center justify-center' style={{ backgroundColor: viewStory.media_type === "text" ? viewStory.background_color : "#000000" }}>
            ...
            {/* user infro  */}
            <div className='absolute top-4 left-4 flex items-center space-x-3 py-2 px-4 sm:p-4 sm:px-8 backdrop-blur-2xl rounded bg-black/50'>
                <img src={viewStory.user?.profile_picture} className='sm:size-8 rounded-full object-cover border border-white' alt="" />
