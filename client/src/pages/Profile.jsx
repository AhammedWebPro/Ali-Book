                ...
                <>
                 {post.image_urls.map((img, index) => (
                  <Link target='_blank' to={img} key={index} className='relative group'>
                    <img src={img} alt="" key={index} className='w-64 aspect-video object-cover' />
                    <p className=' absolute bottom-0 right-0 text-xs p-1 px-3 backdrop-blur-xl text-white opacity-0 group-hover:opacity-100 transition duration-100'>Posted {moment(post.createAdt).fromNow()}</p>
                  </Link>
                 ))}
                </>
              ))
